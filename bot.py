...
class VintedMonitor:
    ...
    def find_items_recursive(self, obj, depth=0, max_depth=10):
        ...
        if isinstance(obj, dict):
            # Se troviamo 'items' che è una lista con elementi che hanno 'id'
    ...
    def fetch_vinted_items(self, url):
        ...
        try:
            ...
            for idx, script in enumerate(scripts):
                ...
                # Se lo script contiene "items" o "catalog", è probabilmente quello giusto
                if 'items' in script_content or 'catalog' in script_content:
                    logger.info(f"🎯 Script #{idx} contiene 'items' o 'catalog', analizzo...")
                ...
                patterns = [
                    ...
                    if matches:
                        logger.info(f"✅ Pattern '{pattern_name}' trovato! Matches: {len(matches)}")
                    
                    for match in matches:
                        try:
                            logger.info(f"🔄 Tento parsing JSON da pattern '{pattern_name}'...")
                            ...
                            if json_str.startswith('{'):
                                ...
                                logger.info(f"📦 JSON parsato con successo! Chiavi: {list(data.keys())[:10]}")
                                ...
                                if 'catalog' in data:
                                    logger.info(f"🔍 Trovato 'catalog' nelle chiavi")
                                    if isinstance(data['catalog'], dict) and 'items' in data['catalog']:
                                        items_data = data['catalog']['items']
                                        logger.info(f"✅ Trovati items in catalog.items: {len(items_data) if isinstance(items_data, list) else 'non-list'}")
                                
                                # Struttura 2: items diretti
                                if not items_data and 'items' in data:
                                    items_data = data['items']
                                    logger.info(f"✅ Trovati items diretti: {len(items_data) if isinstance(items_data, list) else 'non-list'}")
                                
                                # Struttura 3: search.items
                                if not items_data and 'search' in data and isinstance(data['search'], dict) and 'items' in data['search']:
                                    items_data = data['search']['items']
                                    logger.info(f"✅ Trovati items in search.items: {len(items_data) if isinstance(items_data, list) else 'non-list'}")
                                
                                # Struttura 4: Cerca ricorsivamente in tutto il JSON
                                if not items_data:
                                    logger.info("🔍 Cerco 'items' ricorsivamente...")
                                    items_data = self.find_items_recursive(data)
                                    if items_data:
                                        logger.info(f"✅ Trovati items ricorsivamente: {len(items_data)}")
            ...
            if not items:
                logger.warning("⚠️ JSON non trovato in nessuno script, provo parsing HTML...")
                
                # Salva un sample dell'HTML per debug (solo primi 2000 caratteri)
                html_sample = str(soup)[:2000]
                logger.info(f"📄 HTML Sample (primi 2000 char): {html_sample}")
                ...
                # Prova anche a cercare per attributi data-testid
                item_cards_alt = soup.find_all('div', {'data-testid': re.compile(r'item|product')})
                logger.info(f"📦 Trovati {len(item_cards_alt)} elementi con data-testid")
            
            if not items:
                logger.warning(f"❌ Nessun articolo estratto dall'URL")
                logger.warning(f"💡 Suggerimento: Vinted potrebbe aver cambiato la struttura della pagina")
...
if __name__ == '__main__':
    main(), 'APOLLO_STATE'),
                    (r'<script[^>]*>\s*(\{[^<]+?"items"[^<]+?\})\s*</script>', 'INLINE_JSON'),
                ]
                
                for pattern, pattern_name in patterns:
                    matches = re.findall(pattern, script_content, re.DOTALL | re.MULTILINE)
